...
def sample_cg(model, clf, sched, cls_labels, num_steps, guidance_scale, device="cpu"):
    ...
    ############################
    #alpha_t = sched.alphas[t_idx], abar_t  = sched.alphas_cumprod[t_idx]
    #model(x_t, t_vec, cls_labels, force_uncond=True) returns the unconditional eps --> ϵθ(xt, t, ∅)
    ...
    for i, t_idx in enumerate(timesteps):
        ...
        ############################
        eps_uncond = model(x_t, t_idx, cls_labels, force_uncond=True)
        logits = clf(x_t, t_idx)
        log_py = grad_log_softmax(logits, cls_labels)
        grad = torch.autograd.grad(log_py.sum(), x_t)[0]
        eps_guided = eps_uncond - guidance_scale * torch.sqrt(1 - sched.alphas_cumprod[t_idx]) * grad
        At = 1/torch.sqrt(sched.alphas[t_idx])
        Bt = (1 - sched.alphas[t_idx]) / (torch.sqrt(1 - sched.alphas_cumprod[t_idx])*torch.sqrt(sched.alphas[t_idx]))

        x_prev_mean = At * x_t - Bt * eps_guided
...
def sample_cfg(model, sched, cls_labels, num_steps, guidance_scale, device="cpu"):
    ...
    for i, t_idx in enumerate(timesteps):
        ...
        eps_uncond = model(x_t, t_idx, cls_labels, force_uncond=True)
        eps_cond  = model(x_t, t_idx, cls_labels, force_uncond=False)
