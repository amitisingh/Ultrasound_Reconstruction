...
def pack_flat_to_3ch(A_funcs, x_flat):
    ...
    return torch.stack([x0, zeros, zeros], dim=1) 



def build_model_from_yaml(model_config, device):
    ...
    defaults.update(
        dict(
            ...
            num_channels    = model_config.get("num_channels", 256),
            num_res_blocks  = model_config.get("num_res_blocks", 2),
            learn_sigma     = model_config.get("learn_sigma", True),
            class_cond      = model_config.get("class_cond", False),
    ...
    ckpt_path = model_config["model_path"]
...
class Config:
    def __init__(self):
        ...
        self.parser.add_argument('--out_path', type=str, default="results/dps_us")
        ...
        self.parser.add_argument('--zeta', type=int, default=2.5)
...
class posterior_samplers():
    ...
    def dps(self, x_t, t, model_t, measurement, A_funcs, zeta):
    ...
    def dds(self, x_t, t, model_t, measurement, A_funcs, max_iter):
        with torch.no_grad():
            model_out = self.score_model(x_t, model_t)
            model_out, _ = torch.split(model_out, x_t.shape[1], dim=1)
            x0_hat = self.predict_x0_hat(x_t, t, model_out)
            ...
            x_cg = self.CG_normal_eq(A_funcs, rhs, x0_hat_flat, max_iter) 
...
def main():
    ...
    score_model, diffusion = build_model_from_yaml(model_config, device)
    ...
    for idx_img, ref_img in enumerate(loader):
        ...
        for step in tqdm(pbar):
            time = torch.tensor([step], device=device, dtype=torch.long)

            if conf.ps_type == "DPS":
                x_t = ps_ops.dps(x_t, time, time_map[time], measurement, A_funcs, conf.zeta)
            elif conf.ps_type == "DDS":
                x_t = ps_ops.dds(x_t, time, time_map[time], measurement, A_funcs, conf.max_iters)
            else:
                raise ValueError("Use --ps_type DPS or DDS")
        ...
        out_dir = os.path.join(conf.out_path, "US")
